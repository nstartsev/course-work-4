*      SUBROUTINE BLKTRI (IFLG,NP,N,AN,BN,CN,MP,M,AM,BM,CM,IDIMY,Y,
      SUBROUTINE BLKTRI (IFLG,MP,M,AM,BM,CM,NP,N,AN,BN,CN,IDIMY,Y,
     1                   IERROR,W)
...
C
C USAGE                  CALL BLKTRI (IFLG,MP,M,AM,BM,CM,NP,N,AN,BN,CN,
C                                     IDIMY,Y,IERROR,W)
C
...
C
  ...
  144 CONTINUE
      RETURN
      END
      DOUBLE PRECISION FUNCTION BSRH (XLL,XRR,IZ,C,A,BH,F,SGN)
      implicit real*8 (a-h,o-z)
      DIMENSION       A(*)       ,C(*)       ,BH(*)
      COMMON /CBLKT/  NPP        ,K          ,EPS        ,CNV        ,
     1                NM         ,NCMPLX     ,IK
...
C
         IF (H .EQ. 0.D0) GO TO 108
         IF (ABS(E2(L)) .LE. ABS(C/H)) GO TO 108
         E2(L) = H*E2(L)
         IF (E2(L) .NE. 0.D0) GO TO 105
  108    P = D(L)+F
C
C     ********** ORDER EIGENVALUES **********
C
...
C-----------------------------------------------------------------------
      END
*
      double precision function cdabs(x)
      implicit real*8 (a-h,o-z)
      dimension x(2)
      cdabs=dsqrt(x(1)**2+x(2)**2)
      return
      end
*
      complex*16 function cdsqrt(x)
      implicit real*8 (a-h,o-z)
      dimension x(2)
      a2=dsqrt(x(1)**2+x(2)**2)
      a=dsqrt(a2)
      arg2=datan2(x(2),x(1))
      arg=0.5*arg2
      cr=a*dcos(arg)
...
C
C***********************************************************************
      DOUBLE PRECISION FUNCTION EPMACH (DUM)
      implicit real*8 (a-h,o-z)
      COMMON /VALUE/  V
      EPS = 1.D0
  101 EPS = EPS/10.D0
      CALL STRWRD (EPS+1.D0)
...
C
      PIMACH = 4.D0*DATAN(1.D0)
      RETURN
      END
      DOUBLE PRECISION FUNCTION PPSGF (X,IZ,C,A,BH)
      implicit real*8 (a-h,o-z)
      DIMENSION       A(*)       ,C(*)       ,BH(*)
      SUM = 0.D0
      DO 101 J=1,IZ
  ...
  101 CONTINUE
      ...
      RETURN
      END
      DOUBLE PRECISION FUNCTION PPSPF (X,IZ,C,A,BH)
      implicit real*8 (a-h,o-z)
      DIMENSION       A(*)       ,C(*)       ,BH(*)
      SUM = 0.D0
      DO 101 J=1,IZ
  ...
  101 CONTINUE
      ...
      RETURN
      END
      DOUBLE PRECISION FUNCTION PSGF (X,IZ,C,A,BH)
      implicit real*8 (a-h,o-z)
      DIMENSION       A(*)       ,C(*)       ,BH(*)
      FSG = 1.D0
      HSG = 1.D0
